...
/// Remembers the last-logged `MonoRuntime::init()` failure reason so
/// `try_init` only prints when the reason changes (or on recovery),
/// rather than spamming one eprintln per back-off-gated retry attempt
/// (every `back_off_duration()`, e.g. every 500ms in dev).
static LAST_INIT_ERROR: Mutex<Option<String>> = Mutex::new(None);

fn try_init() -> Option<mono::MonoRuntime> {
    match mono::MonoRuntime::init() {
        Ok(rt) => {
            if let Ok(mut last) = LAST_INIT_ERROR.lock() {
                if last.is_some() {
                    eprintln!("[hearthmirror] MonoRuntime::init succeeded (previous attempts had been failing)");
                }
                *last = None;
            }
            Some(rt)
        ...
        Err(e) => {
            ...
            if let Ok(mut last) = LAST_INIT_ERROR.lock() {
                if last.as_deref() != Some(msg.as_str()) {
                    eprintln!("[hearthmirror] MonoRuntime::init failed: {}", msg);
                    *last = Some(msg);
                }
            }
            None
...
pub async fn get_hearthstone_window() -> napi::Result<Option<HearthstoneWindowResult>> {
    Ok(
        window::get_hearthstone_window().map(|w| HearthstoneWindowResult {
            x: w.x,
            y: w.y,
            width: w.width,
            height: w.height,
...
#[napi]
pub fn set_window_owner_to_hearthstone(native_window_handle: Buffer) -> napi::Result<bool> {
    let hwnd = hwnd_from_native_window_handle(native_window_handle.as_ref())?;
    Ok(window::set_window_owner_to_hearthstone(hwnd))
}

#[napi]
pub fn clear_window_owner(native_window_handle: Buffer) -> napi::Result<bool> {
    let hwnd = hwnd_from_native_window_handle(native_window_handle.as_ref())?;
    Ok(window::clear_window_owner(hwnd))
}
...
pub fn unsubscribe_hearthstone_window_events(subscription_id: u32) -> napi::Result<bool> {
    ...
    let foreground_ok =
        unsafe { UnhookWinEvent(raw_to_hook(subscription.foreground_hook)) }.as_bool();
...
pub async fn is_spectating() -> napi::Result<bool> {
    with_runtime_or(
        || false,
        |rt| futures::executor::block_on(reflection::game_state::is_spectating_internal(rt)),
    )
}
...
pub async fn is_game_over() -> napi::Result<bool> {
    with_runtime_or(
        || false,
        |rt| futures::executor::block_on(reflection::game_state::is_game_over_internal(rt)),
    )
}
...
#[napi]
pub async fn get_collection_diagnostic(
) -> napi::Result<reflection::collection::CollectionDiagnostic> {
    with_runtime_or(reflection::collection::CollectionDiagnostic::zero, |rt| {
        futures::executor::block_on(reflection::collection::get_collection_diagnostic_internal(
            rt,
        ))
    })
}
...
pub async fn get_edited_deck() -> napi::Result<Option<reflection::decks::DeckResult>> {
    with_runtime(|rt| {
        futures::executor::block_on(reflection::edited_deck::get_edited_deck_internal(rt))
    })
}
...
pub async fn get_board_state() -> napi::Result<Option<reflection::board_state::BoardStateResult>> {
    with_runtime(|rt| {
        futures::executor::block_on(reflection::board_state::get_board_state_internal(rt))
    })
}
...
pub async fn get_hand_state() -> napi::Result<Option<reflection::hand_state::HandStateResult>> {
    with_runtime(|rt| {
        futures::executor::block_on(reflection::hand_state::get_hand_state_internal(rt))
    })
}
...
pub async fn get_deck_state() -> napi::Result<Option<reflection::deck_state::DeckStateResult>> {
    with_runtime(|rt| {
        futures::executor::block_on(reflection::deck_state::get_deck_state_internal(rt))
    })
}

#[napi]
pub async fn get_opponent_secrets(
) -> napi::Result<Option<reflection::opponent_secrets::OpponentSecretsResult>> {
    with_runtime(|rt| {
        futures::executor::block_on(reflection::opponent_secrets::get_opponent_secrets_internal(
            rt,
        ))
    })
}
...
pub async fn get_choices() -> napi::Result<Option<reflection::choices::ChoicesResult>> {
    with_runtime(|rt| futures::executor::block_on(reflection::choices::get_choices_internal(rt)))
}

#[napi]
pub async fn get_selected_deck_id(
) -> napi::Result<Option<reflection::selected_deck::SelectedDeckResult>> {
    with_runtime(|rt| {
        futures::executor::block_on(reflection::selected_deck::get_selected_deck_id_internal(rt))
    })
}
...
mod wrapper_tests {
    ...
    fn with_runtime_retries_once_on_assembly_csharp_not_found() {
        ...
        assert_eq!(
            init_calls.get(),
            2,
            "try_init should run on first miss + retry re-init"
        );
        assert_eq!(
            f_calls.get(),
            2,
            "closure should run twice (initial + retry)"
        );
    ...
    fn with_runtime_does_not_retry_on_other_module_not_found() {
        ...
        assert_eq!(
            init_calls.get(),
            1,
            "no retry init on non-AC ModuleNotFound"
        );
    ...
    fn assembly_csharp_canary_predicate() {
        assert!(is_assembly_csharp_not_found(&ScryError::ModuleNotFound(
            "Assembly-CSharp.dll".into()
        )));
        assert!(!is_assembly_csharp_not_found(&ScryError::ModuleNotFound(
            "blizzard.bgsclient.dll".into()
        )));
        assert!(!is_assembly_csharp_not_found(
            &ScryError::MonoNotInitialized
        ));
    }
}
