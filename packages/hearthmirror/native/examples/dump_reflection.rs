...
use hearthmirror_native::reflection::{
    ...
    decks, edited_deck, game_state, hand_state, match_info, medal_info, mulligan, opponent_secrets,
    server,
};
...
fn main() {
    ...
    run_opt(
        ...
        |v| {
            let p = |label: &str, m: Option<&match_info::MatchPlayerResult>| match m {
                Some(p) => format!(
                    "{}{{id={}, side={}, name={:?}, cb={}}}",
                    label, p.id, p.side, p.name, p.cardback_id
                ),
    ...
    match futures::executor::block_on(game_state::get_game_type_internal(&rt)) {
        Ok(v) => {
            let any_set =
                v.game_type.is_some() || v.format_type.is_some() || v.mission_id.is_some();
            print_result(
                &mut tally,
                "getGameType",
    ...
    run_opt(
        ...
        |v| {
            format!(
                "addr={}, port={}, gh={}, ver={}, mission={}",
                v.address, v.port, v.game_handle, v.version, v.mission
            )
        },
        t.elapsed().as_millis(),
    );
    ...
    run_opt(
        ...
        |v| {
            let names: Vec<String> = v
                .iter()
                .take(3)
                .map(|d| format!("{:?} ({} cards)", d.name, d.cards.len()))
                .collect();
            format!(
                "{} decks: [{}{}]",
                v.len(),
    ...
    run_opt(
        ...
        |v| {
            format!(
                "friendly_deck={}, opp_deck_count={}",
                v.friendly_deck.len(),
                v.opposing_deck_count
            )
        },
        t.elapsed().as_millis(),
    );
    ...
    run_opt(
        ...
        |v| {
            ...
            format!(
                "{}, {}",
                g("mulligan", v.mulligan.as_ref()),
                g("general", v.general.as_ref())
            )
        },
        t.elapsed().as_millis(),
    );
