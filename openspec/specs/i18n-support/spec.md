...
The repository SHALL contain bundled locale dictionaries for `en-US` and `zh-CN`. The dictionaries MUST cover visible copy for the current desktop shell, navigation, settings view, deck tracker panel, opponent card panel, deck selection dialog, card image empty/error states, and generic status labels used by those surfaces.
