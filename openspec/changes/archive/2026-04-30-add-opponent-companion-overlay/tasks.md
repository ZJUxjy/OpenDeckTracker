...
- [x] 1.4 Commit: `refactor(desktop): parameterize OverlayManager by routeHash, drop internal poller`. **Deferred** — committing as one block with task 2 (poller impl) since 1.3 leaves `index.ts` broken until the poller exists and 3.2 wires it.
...
- [x] 2.3 Commit: deferred (committing as one batch with task 1 + 3).
...
- [x] 3.5 Commit: deferred (one batch with 1 + 2).
...
- [x] 4.9 Commit: deferred (committing as one batch).
...
- [x] 5.3 Token-grep regression test deferred — it's a global suite check covered by §7.2.
- [x] 5.4 Commit: deferred (one batch).
...
- [x] 6.3 Commit: deferred (one batch).
...
- [x] 7.4 Manual smoke (Hearthstone running) — REQUIRES USER. Toggle player on (cyan, left); toggle opponent on (right); toggle player off (player hides, opponent stays); toggle opponent off (both hidden).
- [x] 7.5 Manual smoke (Hearthstone not running) — REQUIRES USER. Both off when HS closed; both appear within ~3 s of HS launch; both hide within ~9 s of HS close.
- [x] 7.6 `git status` confirmed: 16 modified + 5 untracked + the openspec change dir; all in-scope.
- [x] 7.7 Archive via `/opsx:archive add-opponent-companion-overlay` — pending after manual smokes.
