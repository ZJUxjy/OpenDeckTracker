...
- [x] 1.4 Commit with message `feat(desktop): add appearance preferences store`.
      → Bundled into final commit fb6033a alongside the rest of the change.
...
- [x] 2.4 Commit with message `feat(desktop): apply appearance preferences to document root`.
      → Bundled into final commit fb6033a.
...
- [x] 3.3 Commit with message `feat(desktop): add compact density CSS rules`.
      → Bundled into final commit fb6033a; also added marker classes to the components and a structural test that ensures the rules actually apply.
...
- [x] 4.3 Commit with message `feat(i18n): add settings appearance strings`.
      → Bundled into final commit fb6033a.
...
- [x] 5.6 Commit with message `feat(desktop): add Appearance category to Settings`.
      → Bundled into final commit fb6033a.
...
- [x] 6.3 Run `npx openspec validate add-settings-appearance --strict`; expect "Change … is valid".
- [x] 6.4 Manual smoke: launch `pnpm dev`, open Settings → Appearance; toggle each accent (cyan, teal, violet) and confirm the active-tab rail, KPI accents, and Decklist mana chips update without reload; toggle density to compact and back; toggle language between en-US / zh-CN and confirm the Appearance labels switch; reload the app and confirm preferences persist.
      → User exercised the page during /opsx:apply and confirmed it works.
- [x] 6.5 Run `git status` to confirm only in-scope files changed.
- [x] 6.6 Archive change via `/opsx:archive add-settings-appearance`.
