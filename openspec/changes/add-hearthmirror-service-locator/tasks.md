...
- [ ] 1.5 修改 `find_ac_image_cached` 实现为单行 `self.find_image_cached("Assembly-CSharp.dll")`
- [ ] 1.6 `cargo test -p hearthmirror-native --lib --all-features` 通过；
      `cargo clippy -p hearthmirror-native --lib --all-features -- -D warnings` 通过
- [ ] 1.7 提交 `feat(hearthmirror): cross-image MonoClass lookup`

## 2. Dictionary entry-array layout fix（fixture-backed test 优先 / TDD）

- [ ] 2.1 在 `tests/fixtures/dictionary.rs`（如不存在则创建）写一个
...
- [ ] 2.2 在 `src/collections/dict.rs` 加测试 `iter_entries_layout_verified`：
      用 fixture 构造 5 个 entry，第 1 / 3 个 hash<0；断言返回 `Vec<DictEntry>` 长度 3 + 地址正确
- [ ] 2.3 加测试 `iter_entries_overflow_guard`：fixture _count = 1_000_000
      → 断言 `Err(CollectionOverflow { max: 100 })`
- [ ] 2.4 加测试 `iter_entries_empty_returns_empty`：_count=0 + entries 非 NULL
      → 断言 `Ok(vec![])`
- [ ] 2.5 在 `dict.rs` 顶部用 `const _ENTRIES_OFFSET: u32 = 0x0C; const _COUNT_OFFSET: u32 = 0x20;`
...
- [ ] 2.6 把 `iter_entries` 中的 `dict + 0x14` / `dict + 0x18` 改为 `dict + _ENTRIES_OFFSET` / `dict + _COUNT_OFFSET`
- [ ] 2.7 跑 2.2 / 2.3 / 2.4 测试 → 必须先因旧偏移 fail，然后改完后 pass（TDD red→green）
- [ ] 2.8 `cargo clippy --lib --all-features -- -D warnings` 通过
- [ ] 2.9 提交 `fix(hearthmirror): correct Dictionary entry-array offsets (_count +0x20, _entries +0x0C)`

## 3. ServiceLocator 模块（核心实现）

- [ ] 3.1 在 `src/reflection/field_paths.rs` 新增三个常量：
...
- [ ] 3.2 在 `src/reflection/field_paths.rs` 加 `ServiceManager` / `ServiceLocator` / `ServiceInfo` 字段名常量：
      `FLD_S_RUNTIME_SERVICES`, `FLD_M_SERVICES`, `FLD_SERVICE_TYPE_NAME`,
      `FLD_SERVICE`（即 `<Service>k__BackingField`）
- [ ] 3.3 创建 `src/reflection/service_locator.rs` 实现
...
- [ ] 3.4 在 `src/reflection/mod.rs` 注册新模块 `pub mod service_locator;`
- [ ] 3.5 单元测试 `service_locator_layout_constants`：断言常量值符合
      spec（防止 typo）
- [ ] 3.6 提交 `feat(hearthmirror): add ServiceLocator chain helper`

## 4. MonoRuntime::get_service 缓存层

- [ ] 4.1 在 `RuntimeCache` 加字段
      `services: HashMap<String, RemotePtr>`（key=name, value=service object addr）
- [ ] 4.2 实现 `MonoRuntime::get_service(name) -> Result<Option<MonoObject>, ScryError>`：
...
- [ ] 4.3 单元测试 `get_service_cache_evicts_stale`：
      手动写一个不存在的 addr 到 `cache.services["NetCache"]`，
      然后调 `get_service("NetCache")` 应返回 `Ok(None)` 且 cache 中该 key 被移除
- [ ] 4.4 提交 `feat(hearthmirror): add MonoRuntime::get_service with stale-pointer eviction`

## 5. 改写 NetCache 三个反射方法

- [ ] 5.1 `src/reflection/battle_tag.rs`：
      `runtime.get_singleton(CLS_NET_CACHE.0, CLS_NET_CACHE.1)?`
      → `runtime.get_service(SVC_NET_CACHE)?`；其余字段链不动
- [ ] 5.2 `src/reflection/account_id.rs`：同上替换
- [ ] 5.3 `src/reflection/medal_info.rs`：同上替换
- [ ] 5.4 检查 `src/lib.rs` 的 `getBattleTag` / `getAccountId` / `getMedalInfo` 三个 napi
      函数签名与返回类型字面**没有变化**（grep 比对前后）
- [ ] 5.5 `cargo clippy -p hearthmirror-native --all-features -- -D warnings` 通过
- [ ] 5.6 `cargo test -p hearthmirror-native --lib --all-features` 通过
- [ ] 5.7 提交 `refactor(hearthmirror): route NetCache reflectors through ServiceLocator`

## 6. 实战验证 + 文档收口

- [ ] 6.1 在游戏运行的环境下 `cargo run --release --example dump_reflection`：
...
- [ ] 6.2 把实测 dump_reflection 输出的关键统计（"X OK / Y null / Z ERR" 行）
      写入 `docs/spikes/0003-hearthmirror-reflection-runtime-validation.md`
      新加的 `## Run 9` 段
- [ ] 6.3 在同一个 Run 9 段记录三个示例返回（battle tag、accountId hi/lo 截断、
      medalInfo 当前赛季 leagueId / starLevel），并标注"R-16 closed"
- [ ] 6.4 在 Run 9 末尾列出仍 null 的方法名 + 暂定后续归宿（R-17 / R-18）
- [ ] 6.5 提交 `docs(spikes): record post-ServiceLocator reflection bridge state`
