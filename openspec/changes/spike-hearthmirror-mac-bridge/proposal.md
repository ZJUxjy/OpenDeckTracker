...
3. **`CGWindowListCopyWindowInfo` 能否拿到 Hearthstone 桌面 / 全屏两种状态下的 frame？**（fullscreen 用 frame≈display-bounds 启发式打底，AX 真值检测放 Phase 1。）
...
- 临时新建 `packages/hearthmirror-mac-spike/`（macos 分支独有，**不影响 Windows**）：
  - `Cargo.toml` 用 napi-rs 3.x，target 仅 `aarch64-apple-darwin`；macOS-only deps 走 `[target."cfg(target_os = \"macos\")".dependencies]` 包：`mach2` / `libproc` / `core-foundation` / `core-graphics`（**不引 `objc2` 系列** —— spike 的 fullscreen 用启发式，AX 检测留 Phase 1）。
  - `src/lib.rs` 暴露两个 `#[napi]` async 函数：
    - `spike_read_macho()` — 找 Hearthstone PID → `task_for_pid` → `task_info(TASK_DYLD_INFO)` 走 dyld_all_image_infos → `mach_vm_read_overwrite` 16 字节，返回 `(pid, base, hex_string)`。
    - `spike_read_hearthstone_window()` — `CGWindowListCopyWindowInfo` 找 owner == `Hearthstone` 的 layer-0 窗口拿 frame，加 frame≈`CGDisplayBounds(CGMainDisplayID())` 的启发式 fullscreen，返回 `(x, y, width, height, fullscreen)`。
  - `entitlements.dev.plist` 必须同时含 task_for_pid 路径（`cs.debugger` + `get-task-allow`）和 Electron Hardened Runtime 路径（`cs.allow-jit` + `cs.allow-unsigned-executable-memory` + `cs.disable-library-validation`），否则 re-sign 后 dev Electron 自己启动失败。
...
- 在 `apps/desktop/package.json` 的 `devDependencies` 临时加 `"@hdt/hearthmirror-mac-spike": "workspace:*"`（spike 出口前删除）。
- 写一个 `scripts/codesign-mac-spike.sh` 帮 spike `.node` + dev Electron.app ad-hoc 签名（应用 entitlements）。
- 用户在 Apple Silicon 真机上跑 3 个场景（见 [`docs/spikes/0006-hearthmirror-mac-spike.md`](../../../docs/spikes/0006-hearthmirror-mac-spike.md) §Acceptance Criteria）：
  ...
  - **场景 D**：HS 运行 + 桌面 / 主显示器尺寸分别测一次 → `[mac-spike:window] OK: { width, height, fullscreen }` 与实际肉眼匹配。
  - 原计划的 **场景 C**（未签名 → task_for_pid 失败）在自我审查中被识别为方法论错误（`task_for_pid` 检查 calling Electron 而非 addon），已抛弃；签名链路靠场景 A 正向闭环 + Phase 4 release packaging 的 notarization → install 链路覆盖。
...
- 本 change 必须在 macOS 实机上跑（Apple Silicon + macOS 12+ + Hearthstone Mac 客户端）；agent 可以完成全部代码 / 文档骨架，但 §5-§6 的真机验证步骤需要用户配合执行并把 stdout 反馈回来。
