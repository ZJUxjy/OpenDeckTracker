...
function DeckRow({
  ...
  return (
    <div
      className="flex items-center gap-3 p-3 bg-bg-2 border border-border hover:border-border-hi rounded-md"
    ...
    >
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <button
            ...
            className="p-1 rounded hover:bg-bg-3 text-text-dim hover:text-text"
          >
            <MoreVertical size={18} />
          </button>
        </DropdownMenu.Trigger>
        <DropdownMenu.Portal>
          <DropdownMenu.Content className="z-50 min-w-[8rem] bg-bg border border-border rounded-md shadow-xl py-1 text-sm text-text">
            <DropdownMenu.Item
              onSelect={() => onEdit(deck.id)}
              className="px-3 py-1.5 hover:bg-bg-3 outline-none cursor-pointer"
            >
      ...
      <AlertDialog.Root open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialog.Portal>
          ...
          <AlertDialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[400px] bg-bg border border-border rounded-md p-6 text-text">
