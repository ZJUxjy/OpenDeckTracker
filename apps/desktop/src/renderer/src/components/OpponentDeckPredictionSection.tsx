    ...
    <div
      ...
      className={`flex items-center gap-2 px-2 py-1.5 rounded-sm cursor-pointer transition-colors hover:bg-accent-dim/30 hover:border-accent/50 ${
        isActive
          ...
          : isTop
            ? 'bg-bg-3 border border-border-hi'
            : 'bg-bg-2/60 border border-transparent'
      }`}
    >
...
function DeckPopup({ prediction, anchor, playedCounts, onClose }: DeckPopupProps): ReactElement {
  ...
  return createPortal(
    <div
      ...
      className="bg-bg-2 border border-border-hi rounded-md shadow-2xl overflow-hidden flex flex-col"
    >
      <header className="px-3 py-2 border-b border-border bg-bg-3">
...
function DeckPopupRow({
  ...
  return (
    <div
      ...
      className={clsx(
        'relative overflow-hidden text-sm border-b border-border last:border-b-0 transition-colors hover:bg-bg-3 hover:shadow-[inset_3px_0_0_var(--accent)]',
