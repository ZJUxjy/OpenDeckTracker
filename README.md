...
**v0.7.1** is the latest public release. Two install forms available:

**v0.7.1** 是最新公开版本，提供两种安装形态：
...
Windows installations from v0.7.0 onward support in-app updates under **Settings → About**. From v0.7.1, Windows portable ZIP builds also support updates: download, confirm restart, then replace files automatically. Existing v0.7.0 ZIP users need one manual upgrade to v0.7.1. Development builds do not update themselves.

从 v0.7.0 起，Windows 安装版支持在 **设置 → 关于** 中更新。从 v0.7.1 起，ZIP 便携版也支持点击下载、确认重启后自动替换文件。已有 v0.7.0 ZIP 用户需先手动升级到 v0.7.1 一次；开发版不支持自动更新。维护者见 [发版指南](docs/RELEASING.md)。
