...
6. **优先用 `loaded_images` 而非 `domain_assemblies`**：spike 02 证实前者在 §7.2 偏移上工作；后者已漂移。HDT.js 业务上需要的是 `MonoImage*`，从 `loaded_images` 链表更直接。
   ...
   > **实施记录（2026-04-20，翻案）**：由 [`add-hearthmirror-offset-probing`](../../openspec/changes/add-hearthmirror-offset-probing/) 修正为"用 `MonoDomain.domain_assemblies = +0x58` GSList 直接走 `MonoAssembly.image` (`+0x48`)"。Spike 02 观察到的 "`domain_assemblies` 为 NULL" 是因为当时 `Rewrite_Design.md` §7.2 的 `+0x0C` 偏移在这个 build 已漂移到 `+0x58`，`OffsetProber` 配合 `unity-2021.3.json` 的 baseline 把该字段指对了。随后 [`add-hearthmirror-image-walking`](../../openspec/changes/add-hearthmirror-image-walking/) 用 `MonoImage.class_cache = +0x35C` 处的 `MonoInternalHashTable<MonoClass*>` 做 token 哈希查表，彻底替换了 5b 阶段的 `class_def_table` 启发式 byte-scan — 这是 `MonoRuntime::find_class` 的最终形态。遗憾的是 `loaded_images` 链路在 5f 后未被使用（业务层走 `find_class → MonoImage::find_class` 已足够），约束 #6 的"优先"限定词仅作为历史参考保留。
...
**2026-04-20** (`add-hearthmirror-image-walking`): F-11 的 `class_def_table` 启发式被替换为 `MonoImage.class_cache` 嵌入式 `MonoInternalHashTable` walk。新模块 `packages/hearthmirror/native/src/mono/image.rs` 提供 `MonoImage<'r>::{name, enumerate_classes, find_class}`，后两者遍历 bucket 链（next 指针在 `class.next_class_cache = +0xA0`）。`MonoClassRef` 追加 `parent / fields_recursive / find_field`（32 层 hierarchy cap + "child overrides parent" merge），对应新 `MonoFieldDef` 结构；`MonoObject::find_field` 通过 `vtable → class → find_field` 做继承感知的字段查找。`runtime.rs::find_class` 重写为一次性 `MonoImage::find_class` 调用；旧 70+ 行的 `probe_class_def_table_offset` / `find_class_def_table_offset_cached` / `RuntimeCache::class_def_table_offset` 全部删除；`ScryError::ClassNotFound` 升级为 `{ namespace, name }` 双字段。Lib 测试 56 → 71（+4 field + 6 class + 3 image + 2 ClassNotFound display），新增 3 个真机集成测试 `integration_image_walking.rs`（`enumerate_classes_returns_assembly_csharp_classes` / `find_class_collection_manager` / `find_class_unknown_class_returns_class_not_found`）。约束 #5 (disasm-based probing) + 约束 #6 (domain_assemblies 路径) 合力兑现，12 个 `#[napi]` 反射方法现在有一条无启发式的 Mono runtime → class 解析主干路径。
