...
This decision was validated by spike [`docs/spikes/0001-hearthmirror-spike-report.md`](../spikes/0001-hearthmirror-spike-report.md) on 2026-04-19. Both Acceptance Criteria scenarios passed in the local environment:
...
普通用户权限即可调用 `OpenProcess`，未发生 Defender 拦截或 EAC 反作弊干扰。完整观察记录与对正式实现的 10 条建议见 spike report。
